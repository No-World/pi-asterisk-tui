...
> 对话记录保持 Claude 风格的紧凑：思考块折叠为一行 ✻ 标签（pi 的 ctrl+t 开关），
> `/thinking` 可随时打开滚动视图查看任意一条完整思维链，Working 指示器实时显示工具调用次数，
...
- **Claude 风格紧凑对话**：思考块折叠为一行 ✻ 标签，`/thinking` 打开滚动视图查看完整思维链，Classic 底栏显示单轮摘要（思考时长 + 工具调用数），Working 指示器实时统计工具调用
