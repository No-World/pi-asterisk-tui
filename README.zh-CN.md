...
> **标签可点击**——点开对应消息的思维链，再点一下收回；`/thinking` 可随时打开滚动视图查看任意
> 一条完整思维链，Working 指示器实时显示工具调用次数，Classic 底栏完成后显示单轮摘要
> （✻ 8s · 2 shell commands）。
>
> 安装 **本 Fork**：
>
...
- **Claude 风格紧凑对话**：思考块折叠为一行 ✻ 标签，全屏模式下点击标签展开/收回对应思维链，`/thinking` 打开滚动视图查看完整思维链，Classic 底栏显示单轮摘要（思考时长 + 工具调用数），Working 指示器实时统计工具调用
