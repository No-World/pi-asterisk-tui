...
> 对话记录保持 Claude 风格的紧凑：思考块折叠为一行 ✻ 标签（pi 的 ctrl+t 开关），Working 指示器
> 实时显示工具调用次数，Classic 底栏完成后显示单轮摘要（✻ 8s · 2 shell commands）。
>
> 安装 **本 Fork**：
>
> ```bash
...
- **Claude 风格紧凑对话**：思考块折叠为一行 ✻ 标签，Classic 底栏显示单轮摘要（思考时长 + 工具调用数），Working 指示器实时统计工具调用
