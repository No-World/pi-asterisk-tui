...
4. **环境行**——识别的运行环境与版本、MCP 服务器计数（仅当实际安装了
   pi-mcp-adapter 时统计）、内存占用、压缩次数、pi 版本。
...
- 双语 `/open-tui` 设置面板（英文 / 简体中文）：底栏段落、HUD 开关、遥测字段、图标
  模式（nerd / ascii / auto）、光标样式、全屏滚轮速度——含命名风格预设（hud /
  classic / custom）。
